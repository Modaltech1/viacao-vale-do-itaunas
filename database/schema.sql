...
create table if not exists public.motoristas (
  ...
  criado_por uuid references public.perfis(id) on delete set null,
  atualizado_por uuid references public.perfis(id) on delete set null,
  constraint motoristas_status_profissional_check check (status_profissional in ('ativo', 'inativo', 'afastado', 'inapto'))
);
...
begin
  ...
  loop
    ...
    select v_id, s.id, s.nome, s.categoria, v_servico_valor,
      auth.uid(), auth.uid()
...
revoke execute on function public.fn_criar_manutencao_concluida(uuid,text,text,timestamptz,timestamptz,numeric,uuid,text,text,jsonb,jsonb) from public, anon;
...
grant execute on function public.fn_criar_manutencao_concluida(uuid,text,text,timestamptz,timestamptz,numeric,uuid,text,text,jsonb,jsonb) to authenticated;
...
-- =========================================================
-- 18. SINISTROS / AVARIAS OPERACIONAIS
-- =========================================================
